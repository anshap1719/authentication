...
// Server lists the facebook service endpoint HTTP handlers.
type Server struct {
	Mounts            []*MountPoint
...
// New instantiates HTTP handlers for all the facebook service endpoints.
func New(
	e *facebook.Endpoints,
	mux goahttp.Muxer,
	...
	return &Server{
		Mounts: []*MountPoint{
			{"RegisterURL", "GET", "/facebook/register-start"},
			{"AttachToAccount", "POST", "/facebook/attach"},
			{"DetachFromAccount", "POST", "/facebook/detach"},
			{"Receive", "GET", "/facebook/receive"},
			{"CORS", "OPTIONS", "/facebook/register-start"},
			{"CORS", "OPTIONS", "/facebook/attach"},
			{"CORS", "OPTIONS", "/facebook/detach"},
			{"CORS", "OPTIONS", "/facebook/receive"},
		},
...
func (s *Server) Service() string { return "facebook" }
...
// Mount configures the mux to serve the facebook endpoints.
...
// MountRegisterURLHandler configures the mux to serve the "facebook" service
// "register-url" endpoint.
func MountRegisterURLHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := handleFacebookOrigin(h).(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("GET", "/facebook/register-start", f)
}

// NewRegisterURLHandler creates a HTTP handler which loads the HTTP request
// and calls the "facebook" service "register-url" endpoint.
func NewRegisterURLHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, goa.ServiceKey, "facebook")
...
// MountAttachToAccountHandler configures the mux to serve the "facebook"
...
func MountAttachToAccountHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/facebook/attach", f)
}

// NewAttachToAccountHandler creates a HTTP handler which loads the HTTP
// request and calls the "facebook" service "attach-to-account" endpoint.
...
// MountDetachFromAccountHandler configures the mux to serve the "facebook"
...
func MountDetachFromAccountHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/facebook/detach", f)
}

// NewDetachFromAccountHandler creates a HTTP handler which loads the HTTP
// request and calls the "facebook" service "detach-from-account" endpoint.
...
// MountReceiveHandler configures the mux to serve the "facebook" service
// "receive" endpoint.
func MountReceiveHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/facebook/receive", f)
}

// NewReceiveHandler creates a HTTP handler which loads the HTTP request and
// calls the "facebook" service "receive" endpoint.
...
func MountCORSHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("OPTIONS", "/facebook/register-start", f)
	mux.Handle("OPTIONS", "/facebook/attach", f)
	mux.Handle("OPTIONS", "/facebook/detach", f)
	mux.Handle("OPTIONS", "/facebook/receive", f)
}
...
// handleFacebookOrigin applies the CORS response headers corresponding to the
// origin for the service facebook.
func handleFacebookOrigin(h http.Handler) http.Handler {
